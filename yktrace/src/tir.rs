...
use ykpack::{BasicBlockIndex, Body, Decoder, DefId, Pack, Statement, Terminator};
...
impl TirTrace {
    ...
    /// Returns Err if a DefId is enountered for which no SIR is available. In the error case, the
    /// trace built thus-far is returned inside the error.
    ///
    /// FIXME: This is not the final "intended" API. Noramlly we wouldn't care about the trace
    /// built thus-far in an error case, but since we have no sensible way to stop the tracer at
    /// the right time, inevitably the trace shoots off into code which has no SIR. In turn this
    /// invalidates the trace. We only return the trace built so far for now so that we have
    /// *something* to test in the interim.
    pub(crate) fn new(trace: &'_ dyn SirTrace) -> Result<Self, Self> {
        ...
        for blk_idx in 0..num_locs {
            ...
            match body.blocks[shadow_bb_idx_usize].term {
                Terminator::Call{ret_bb, ..} => debug_assert!(ret_bb == Some(BasicBlockIndex::try_from(user_bb_idx_usize).unwrap())),
...
mod tests {
    ...
    use crate::{start_tracing, TracingKind, TirTrace};
    use test::black_box;
    ...
    fn nonempty_tir_trace() {
        ...
        println!("THis is a teds");
        let sir_trace = tracer.t_impl.stop_tracing().unwrap();
        // FIXME In the future tracing will not fail.
        let tir_trace = TirTrace::new(&*sir_trace).err().unwrap();
        assert_eq!(res, 15);
