...
pub enum Ty {
    ...
    Tuple(TupleTy),
    /// An array type.
    /// FIXME byte_size and align can be computed from elem_ty and len, but requires some refactoring..
    Array{elem_ty: TypeId, len: usize, byte_size: usize, align: usize},
    /// A slice type.
    Slice(TypeId),
...
impl Display for Ty {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Ty::Array{elem_ty, len, ..} => write!(f, "[({}, {}); {}]", elem_ty.0, elem_ty.1, len),
...
impl Ty {
    pub fn size(&self) -> u64 {
        match self {
            ...
            Ty::Array{byte_size, ..} => u64::try_from(*byte_size).unwrap(),
            _ => todo!(),
        }
    }
    ...
    pub fn align(&self) -> u64 {
        match self {
            ...
            Ty::Bool => u64::try_from(mem::size_of::<bool>()).unwrap(),
            Ty::Array{align, ..} => u64::try_from(*align).unwrap(),
            _ => todo!("{:?}", self),
        }
    }
}
...
pub enum IPlace {
    ...
    Indirect {
        ...
        ptr: Ptr,
        /// Offsetting that needs to occur *after* the pointer has been dereferenced.
...
impl Display for IPlace {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Indirect{ptr, post_offs, ty: _ty} => write!(f, "*({})+{}", ptr, post_offs),
...
impl IPlace {
    ...
    pub fn local(&self) -> Option<Local> {
        match self {
            Self::Val{local, ..} | Self::Indirect{ptr: Ptr{local, ..}, ..} => Some(*local),
    ...
    /// stored to.
    pub fn to_indirect(&self) -> IPlace {
        match self {
            Self::Val{local, offs, ty} => {
                let ptr = Ptr{local:*local, offs: *offs};
                IPlace::Indirect{ptr, post_offs: 0, ty: *ty}
            },
            Self::Const{..} => todo!(),
            Self::Indirect{..} | Self::Unimplemented(_) => unreachable!(),
        }
    }
}
...
pub enum Statement {
    ...
    MkRef(IPlace, IPlace),
    /// FIXME
    DynOffs{dest: IPlace, base: IPlace, idx: IPlace, scale: u32},
...
impl Statement {
    ...
    pub fn maybe_defined_locals(&self) -> Vec<Local> {
        ...
        match self {
            ...
            Statement::DynOffs{dest, ..} => Self::maybe_push_local(&mut ret, dest.local()),
    ...
    pub fn used_locals(&self) -> Vec<Local> {
        ...
        match self {
            ...
            Statement::DynOffs{dest, base, idx, ..} => {
                Self::maybe_push_local(&mut ret, dest.local());
...
impl Display for Statement {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Statement::DynOffs{dest, base, idx, scale, ..} => write!(f, "{} = dynoffs({}, {}, {})", dest, base, idx, scale),
