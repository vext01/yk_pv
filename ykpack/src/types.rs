...
pub enum CallOperand {
    /// A call to a binary symbol.
    Fn(Option<String>), // FIXME get rid of option.
...
impl Display for CallOperand {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CallOperand::Fn(sym_name) => {
                let sym_name_str = sym_name.as_ref().map(|s| s.as_str())
                    .or(Some("<unknown symbol>")).unwrap();
                write!(f, "sym_name={}", sym_name_str)
            }
